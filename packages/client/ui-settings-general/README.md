...
The settings panel uses a balanced desktop split between its section rail and scrollable content rather than sizing itself from whichever section is active. On compact viewports the rail becomes a horizontal section switcher and the content remains a single scrolling column, so feature rows never inherit an unusably narrow text measure.
