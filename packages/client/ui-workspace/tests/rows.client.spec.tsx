...
describe('workspace browser rows', () => {
  it('omits only an empty leading status slot in the hierarchy-free flat list', () => {
    const idle: SessionNode = {
      ...
      runningSubagentCount: 0, completed: false, updatedAt: 0, depth: 0,
    }
    ...
    const renderRow = (over: Partial<SessionNode>) => render(
      <SessionNodeItem
        node={{
          ...
          runningSubagentCount: 0, completed: false, updatedAt: 0, depth: 0, ...over,
        }}
  ...
  it('shows descendant activity without describing an idle parent as running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 2, completed: false, updatedAt: 0, depth: 0,
      }
  ...
  it('keeps descendant activity secondary while the parent is running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 1, completed: false, updatedAt: 0, depth: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
      const row = screen.getByRole('treeitem')
  ...
  it('keeps child activity as a secondary status while user attention is primary', () => {
    const node: SessionNode = {
      ...
      running: false, runningSubagentCount: 1, completed: false, updatedAt: 0, depth: 0,
    }
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
    const row = screen.getByRole('treeitem')
  ...
  it.each([
    ...
    try {
      const node: SessionNode = {
        ...
        pendingInteraction, running: true, runningSubagentCount: 0, completed: false, updatedAt: 0, depth: 0,
      }
  ...
  it('completed hover card shows the Completed status line', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 0, completed: true, updatedAt: 0, depth: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
      fireEvent.pointerEnter(screen.getByRole('treeitem').parentElement as HTMLElement)
  ...
  it('indents subagent rows and still wires their drag events', () => {
    const top: SessionNode = {
      id: sid('top'), title: 'Parent', blank: false, running: false,
      runningSubagentCount: 0, completed: false, updatedAt: 0, depth: 0,
    }
    const child: SessionNode = {
      id: sid('child'), title: 'Child', blank: false, running: false,
      runningSubagentCount: 0, completed: false, updatedAt: 0, depth: 1,
    }
    const active = dragProps({ active: true })
    const { rerender } = render(
      <SessionNodeItem node={top} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} drag={active} t={t} />,
    )
    expect(screen.getByRole('treeitem').getAttribute('style')).toBeNull()

    rerender(
      <SessionNodeItem node={child} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} drag={active} t={t} />,
    )
    const row = screen.getByRole('treeitem')
    // One level of indentation; the row still participates in drag so the
    // browser can move the whole parent block.
    expect(row.getAttribute('style')).toContain('padding-left: 30px')
    expect(row.getAttribute('draggable')).toBe('true')
    fireEvent.dragStart(row, { dataTransfer })
